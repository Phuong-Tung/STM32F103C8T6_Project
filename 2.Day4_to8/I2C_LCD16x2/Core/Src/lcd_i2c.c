...
static void lcd_send(uint8_t data, uint8_t rs) {
    ...
    data_arr[0] = upper | 0x0C | (rs ? 0x01 : 0x00); // En=1, Rs=?, Backlight=1
    data_arr[1] = upper | 0x08 | (rs ? 0x01 : 0x00); // En=0
    data_arr[2] = lower | 0x0C | (rs ? 0x01 : 0x00);
    data_arr[3] = lower | 0x08 | (rs ? 0x01 : 0x00);
